...
export default function SearchTrigger({ placeholder = 'Search…', shortcut = '⌘K', onClick, className }) {
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'flex h-[32px] w-full max-w-[480px] items-center gap-8 rounded-full border border-line-subtle bg-content px-12 text-start shadow-stroke-faint transition-colors hover:bg-hover',
        focusRing,
        className,
      )}
    >
...
export function CommandInput({ value, onChange, placeholder, icon: Icon = SearchIcon, className, inputRef, ...props }) {
  return (
    <div className={cn('flex h-[44px] items-center border-b border-line px-8', className)}>
      <Icon size={14} strokeWidth={1.5} className="ms-[9px] shrink-0 text-pill-muted" />
      <input
        ref={inputRef}
        type="text"
        role="searchbox"
        ...
        className={cn(
          'h-[22px] min-w-0 flex-1 bg-transparent ps-8 text-[15px] font-[450] leading-[22px] text-body',
          focusRing,
        )}
        {...props}
      />
