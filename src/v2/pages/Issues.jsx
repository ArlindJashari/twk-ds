...
import { CaretDown, UserAvatarIcon } from '../../components/icons.jsx'
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-8">
        <div className="pt-2">
          <GroupHeader
            ...
            leading={(
              <>
                <CaretDown size={10} className="shrink-0 text-[hsl(var(--muted-foreground))]" />
          ...
          <div className="flex flex-col">
            {todoIssues.map((issue) => (
              <IssueRow
                ...
                title={issue.title}
                date={issue.date}
                href={v2Href(`/${issue.id}`)}
                priority={<PriorityIcon level={issue.priority ?? 'none'} />}
                status={<StatusIcon status="todo" />}
                assignee={(
                  <span className="grid size-[18px] place-items-center rounded-full text-[hsl(var(--muted-foreground))]">
